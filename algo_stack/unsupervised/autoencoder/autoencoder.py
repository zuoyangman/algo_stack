...
class Autoencoder(BaseEstimator, TransformerMixin):
    """Symmetric encoder–decoder with a bottleneck hidden layer.

    Minimises mean squared reconstruction error ``||x − x̂||²``.
    ...
    def _encode(self, X: np.ndarray) -> np.ndarray:
        a1 = activations.forward(self.activation, X @ self.encoder_W1_ + self.encoder_b1_)
        return a1 @ self.encoder_W2_ + self.encoder_b2_

    def _decode(self, Z: np.ndarray) -> np.ndarray:
        a1 = activations.forward(self.activation, Z @ self.decoder_W1_ + self.decoder_b1_)
        return a1 @ self.decoder_W2_ + self.decoder_b2_

    def _forward(self, X: np.ndarray) -> tuple[np.ndarray, dict]:
        enc_pre = X @ self.encoder_W1_ + self.encoder_b1_
        enc_h = activations.forward(self.activation, enc_pre)
        code = enc_h @ self.encoder_W2_ + self.encoder_b2_
        dec_pre = code @ self.decoder_W1_ + self.decoder_b1_
        dec_h = activations.forward(self.activation, dec_pre)
    ...
    def _backward(self, recon: np.ndarray, cache: dict) -> list[np.ndarray]:
        ...
        d_dec_pre = d_dec_h * activations.grad(self.activation, cache["dec_h"])
        ...
        d_enc_pre = d_enc_h * activations.grad(self.activation, cache["enc_h"])
    ...
    def reconstruct(self, X: np.ndarray) -> np.ndarray:
        """Decode the bottleneck and return reconstructed inputs."""
